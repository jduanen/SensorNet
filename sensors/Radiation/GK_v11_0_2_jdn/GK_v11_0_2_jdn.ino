/* GeigerKit_Default sketch (v.11.0.2)     SHIPPING    21796 / 877 bytes in 1.0.5               bHogan 6/6/14
 ...
 * It requires the Arduino IDE rel. 1.6.9 or above to compile.
 *
 * FEATURES:
 *
 *
 *--- insert by impexeris ---
 * v.11.0.2 - adaptation modifications are listed below:
 * PIN definitions are customized to work with the SMD based device shield as follows:
 ...
 *  Changes made as off 11/04/16:
 *  - multi click controll through push buttons:
 *    -- click of a "mode" button changes between modes (windows of display as in prevous versions)
 *    -- double-click on a "mode" button switches OFF or ON  the LCD backlight (this is a new feature introduced in hardware which was not available in previous - non-SMD based hardware versions)
 *    -- click on a "tone-zero" button sets tone to lowest value at current activity of registration (as was in the original version)
 *    -- double-click on a "tone-zero" button mutes or unmutes the beeper (in both  click and tone modes). LCD briefly shows "MUTE" in the right lower corner of the LCD when this happens
 *    -- long-click of a "tone-zero" button changes between the primary and secondary tube ratios. LCD briefly shows "PRIM" or "SEC" in the right lower corner of the LCD when this happens. Note (Previous function of choosing ratios by hardware wiring of the micro-controller pins to ground is removed as no more reasonable for SMD version)
 *    
 * --- end of insert by impexeris ---
 * 
 * 
...
MeetAndroid Android;
 ...
 OneButton button1(SEL_BUTTON, true);
 OneButton button2(NULL_BUTTON, true);
...
void setup(){
   ...
   // link the Nullbutton functions.
button1.attachClick(Modetoggle);
button1.attachDoubleClick(Backlighttoggle);
 // Nullbutton.attachLongPressStart(longPressStart1);
 // Nullbutton.attachLongPressStop(longPressStop1);
 // Nullbutton.attachDuringLongPress(longPress1);
  
  // link the Nullbutton functions.
button2.attachClick(OTonebyButton);
button2.attachDoubleClick(MutebyButton);
button2.attachLongPressStart(ToggleRate);
 // Nullbutton.attachLongPressStop(longPressStop2);
 // Nullbutton.attachDuringLongPress(longPress2);
...
#endif  
  attachInterrupt(0,GetEvent,RISING);  // Geiger event on pin 2 triggers interrupt
  pinMode(PULSE_PIN,OUTPUT);            // sets the ditigal pin 19 (A5) to be digital output
...
#if (JDN)
  digitalWrite(LED_PIN, 0);  // start off with backlight off
#else
  Backlighttoggle();
#endif
  ...
  lcd.print(F("v.11.0.2"));               // display the version
#else
  lcd.print(F("   GEIGER KIT"));        // display a simple banner
  lcd.setCursor(0,1);                   // set cursor on line 2
  lcd.print(F("Ver. 11.0.2_JDN"));        // display the version
#endif
...
if (setNullPoint) {  // start/stop alt display mode if button pin is low
//    lastButtonTime = millis();          // reset the period time
...
// This function will be called when the Mode button was pressed 1 time.
void Modetoggle() {
    if(AlarmOn == false || alarmSilence == true) toggleScaler();
    if (AlarmOn == true) {              // maybe it was pressed to silence the alarm
      digitalWrite(ALARM_PIN, LOW);     // turn off alarm 
...
// This function will be called when the Mode button was pressed 2 times in a short timeframe.
void Backlighttoggle() {
toggle = !toggle;
digitalWrite(LED_PIN,toggle);
} // Backlighttoggle

// This function will be called when the NullButton was pressed 1 time.
void OTonebyButton() {
    setNullPoint = false;               // reset the flag
    nullPoint = 1.2 * currentDispCPM;   // set the nullPoint to 120% of the displayed CPM
} // OTonebyButton
...
// This function will be called when the NullButton was pressed 2 times in a short timeframe.
void MutebyButton() {
togglePiezo(!PiezoOn);
clearArea (11,1,5);                 
...
void ToggleRate() {
if (readParam(ISPRIMARYRATE_ADDR)) {
  writeParam(false, ISPRIMARYRATE_ADDR);
  clearArea (11,1,5);                 
      lcd.print(F("  SEC"));              // display parameter change to Secondary rate
      delay(500);
      clearArea (11,1,5);
}
 else {
 writeParam(true, ISPRIMARYRATE_ADDR);
  clearArea (11,1,5);                 
      lcd.print(F(" PRIM"));              // display parameter change to Primary rate
      delay(500);
      clearArea (11,1,5);
}
}

// This function  will produce a single pulse on PULSE_PIN digital pin 19 (A5)
void FirePulse() {
if (isfirepulse) {
